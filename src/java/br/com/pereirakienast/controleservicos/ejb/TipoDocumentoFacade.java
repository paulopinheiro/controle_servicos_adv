...
public class TipoDocumentoFacade extends AbstractFacade<TipoDocumento> {
    ...
    private TipoDocumento getByNome(String nome) throws LogicalException {
        ...
        try {
            ...
            throw new LogicalException("Há uma inconsistência no banco de dados. Dois tipos de documento estão cadastrados com o mesmo nome " + nome);
        }
    }
    ...
    public void excluir(TipoDocumento entity) throws LogicalException {
        if (entity !=null) {
            if (!entity.getListaDocumentosClientes().isEmpty()) {
                throw new LogicalException("Não é possível excluir o tipo de documento " + entity.getNome()+ ", pois existem documentos deste tipo cadastrados para clientes");
            }
            super.remove(entity);
        }
    }
